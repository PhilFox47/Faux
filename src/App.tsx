...
export const VerifiedBadge = ({
  ...
  if (user.account_type === "company") {
    return (
      <span title="Verified Company" className="inline-flex">
        <BadgeCheck
          size={size}
          className="text-yellow-500 fill-yellow-900 flex-shrink-0"
        />
      </span>
    );
  }
  if (user.account_type === "news") {
    return (
      <span title="Verified News" className="inline-flex">
        <BadgeCheck
          size={size}
          className="text-red-500 fill-red-900 flex-shrink-0"
        />
      </span>
    );
  }
  if (user.account_type === "character" && user.is_verified) {
    return (
      <span title="Verified Public Figure" className="inline-flex">
        <BadgeCheck
          size={size}
          className="text-blue-500 fill-blue-900 flex-shrink-0"
        />
      </span>
    );
  }
...
const CharacterSidebarItem = React.memo(
  ({
    ...
    return (
      <div
        className={`flex items-center gap-3 group ${!u.is_active ? "opacity-50 grayscale" : ""}`}
      >
        <div
        ...
        >
          ...
          {u.is_ai === 1 && (
            <div
              className={`absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-gray-900 ${isUserOnline(u) ? "bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]" : "bg-gray-500"}`}
              title={isUserOnline(u) ? "Online" : "Offline"}
            ></div>
          )}
        </div>
        <div
        ...
        >
          <div className="flex items-center gap-1 min-w-0">
            <p className="font-bold truncate text-sm group-hover:underline">
              {u.display_name}
            </p>
        ...
        <div className="flex flex-col gap-1 flex-shrink-0">
          ...
          <div className="flex gap-1">
            <button
              onClick={() => {
                const isAlreadyOpen = activeChat?.id === u.id && !isGroupChat;
                ...
                fetchChatMessages(u.id, false, undefined, isAlreadyOpen);
              }}
        ...
        <div className="bg-gray-900 rounded-2xl p-4 flex-1 flex flex-col overflow-hidden min-h-0">
          ...
          <div className="mb-4 relative">
            <input
              ...
              className="w-full bg-gray-800 text-white px-4 py-2 pr-10 rounded-full outline-none focus:ring-2 focus:ring-orange-500"
            />
            <button
          ...
          <div className="space-y-4 overflow-y-auto flex-1 pr-2 min-h-0">
            ...
            {hasMoreExplore && (
              <div className="flex justify-center py-4">
                <button
                  onClick={() =>
                    fetchExploreUsers(exploreOffset + 20, true, characterSearch)
                  }
...
export default function App() {
  ...
  const [loginBackgroundAvatars, setLoginBackgroundAvatars] = useState<
    string[]
  >([]);
  ...
  const apiFetch = useCallback(
    async (resource: RequestInfo | URL, config?: RequestInit) => {
      ...
      try {
        ...
        if (!response.ok) {
          console.warn(
            `API request failed: ${resource} ${response.status} ${response.statusText}`,
          );
        }
        return response;
      } catch (error) {
  ...
  const [localExploreSearch, setLocalExploreSearch] = useState("");
  useEffect(() => {
    setLocalExploreSearch(characterSearch);
  }, [characterSearch]);
  ...
  const [lastProcessedMsgId, setLastProcessedMsgId] = useState<number | null>(
    null,
  );
  ...
  const [timelineUniverseFilter, setTimelineUniverseFilter] =
    useState<string>("");
  ...
  const [charAccountType, setCharAccountType] = useState<
    "character" | "company" | "news"
  >("character");
  ...
  const [charRunByCharacterId, setCharRunByCharacterId] = useState<
    number | null
  >(null);
  ...
  const [visionModelName, setVisionModelName] = useState(
    "zai-org/glm-5-vision",
  );
  const [apiKey, setApiKey] = useState("");
  const [timezone, setTimezone] = useState("UTC");
  const [allowNsfw, setAllowNsfw] = useState(false);
  const [enablePerformanceLogging, setEnablePerformanceLogging] =
    useState(false);
  ...
  const [imageResolutions, setImageResolutions] = useState<string[]>([
    "4096x4096",
    "2304x4096",
    "4096x2304",
  ]);
  ...
  const [viewingUniverseCharacters, setViewingUniverseCharacters] = useState<
    any[]
  >([]);
  ...
  const [universeActiveTab, setUniverseActiveTab] = useState<
    "characters" | "arcs"
  >("characters");
  ...
  const [profileReferenceImages, setProfileReferenceImages] = useState<
    string[]
  >([]);
  const [profileDescription, setProfileDescription] = useState("");
  const [profileWritingStyle, setProfileWritingStyle] = useState("");
  const [profilePhysicalAppearance, setProfilePhysicalAppearance] =
    useState("");
  const [profileClothingStyle, setProfileClothingStyle] = useState("");
  const [profileArtstyle, setProfileArtstyle] = useState("");
  const [profileUniverseId, setProfileUniverseId] = useState<number | null>(
    null,
  );
  ...
  const [profileRunByCharacterId, setProfileRunByCharacterId] = useState<
    number | null
  >(null);
  ...
  const [hasMoreRelationshipChecks, setHasMoreRelationshipChecks] =
    useState(true);

  const [arcs, setArcs] = useState<any[]>([]);
  const [arcsOffset, setArcsOffset] = useState(0);
  const [hasMoreArcs, setHasMoreArcs] = useState(true);
  const [expandedArcHistories, setExpandedArcHistories] = useState<
  ...
  const fetchApiLogs = useCallback(
    (
      query?: string | React.MouseEvent | React.KeyboardEvent,
      errorOnly?: boolean,
    ) => {
  ...
  const isUserOnline = useCallback(
    (user: any, precalculatedTime?: number) => {
  ...
  const onlineAiCount = useMemo(() => {
    return users.filter(
      (u) => u.is_ai === 1 && isUserOnline(u, currentTimeInMinutes),
    ).length;
  ...
  const handleEditProfile = async (user: any) => {
    ...
    try {
      setProfileReferenceImages(
        user.reference_images ? JSON.parse(user.reference_images) : [],
      );
    } catch (e) {
      setProfileReferenceImages([]);
    }
  ...
  const handleAddRelationship = async (e: React.FormEvent) => {
    ...
    try {
      const res = await apiFetch(
        `/api/users/${editingProfile.id}/relationships`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            user_id_2: newRelUserId,
            description: newRelDesc,
          }),
        },
      );
  ...
  const handleDeleteRelationship = async (otherId: number) => {
    ...
    setProfileRelationships((prev) =>
      prev.filter((r) => r.user_id_2 !== otherId),
    );

    try {
      await apiFetch(
        `/api/users/${editingProfile.id}/relationships/${otherId}`,
        {
          method: "DELETE",
        },
      );
  ...
  const refreshViewingArcs = async () => {
    ...
    if (viewingUniverse && viewingUniverse.id !== -1) {
      const arcsRes = await apiFetch(
        `/api/universes/${viewingUniverse.id}/arcs`,
      );
      if (arcsRes.ok) {
        const arcs = await arcsRes.json();
  ...
  const fetchExploreUsers = useCallback(
    (offset = 0, append = false, search = "") => {
      ...
      apiFetch(
        `/api/users?limit=${limit}&offset=${offset}&is_ai=true${searchParam}`,
      )
        .then((r) => r.json())
        .then((data) => {
          if (append) {
          ...
          if (beforeId) {
            ...
            setChatMessages((prev) => {
              if (silent && prev.length > 0) {
                const hasContentChanged = data.some(
                  (m: any, i: number) =>
                    m.content !== prev[i]?.content ||
                    m.image_url !== prev[i]?.image_url ||
                    m.image_request_status !== prev[i]?.image_request_status ||
                    m.is_image_request !== prev[i]?.is_image_request,
                );
                if (!hasContentChanged && data.length === prev.length)
                  return prev;
              }
              return data;
            });
    ...
    apiFetch("/api/settings")
      ...
      .then((data) => {
        if (data) {
          ...
          if (data.vision_model_name)
            setVisionModelName(data.vision_model_name);
          if (data.timezone) setTimezone(data.timezone);
          if (data.api_key !== undefined) setApiKey(data.api_key);
          if (data.allow_nsfw !== undefined)
            setAllowNsfw(data.allow_nsfw === 1);
          if (data.enable_performance_logging !== undefined)
            setEnablePerformanceLogging(data.enable_performance_logging === 1);
          if (data.prob_post !== undefined) setProbPost(data.prob_post);
          if (data.prob_comment !== undefined)
            setProbComment(data.prob_comment);
          if (data.prob_message !== undefined)
            setProbMessage(data.prob_message);
          if (data.prob_favorite_dm !== undefined)
            setProbFavoriteDm(data.prob_favorite_dm);
          if (data.cross_universe_prob !== undefined)
            setCrossUniverseProb(data.cross_universe_prob);
          if (data.show_internal_thoughts !== undefined)
            setShowInternalThoughts(data.show_internal_thoughts === 1);
  ...
  const handleUpdateSettings = async (newSettings: any) => {
    ...
    if (newSettings.prob_comment !== undefined)
      setProbComment(newSettings.prob_comment);
    if (newSettings.prob_message !== undefined)
      setProbMessage(newSettings.prob_message);
    if (newSettings.prob_favorite_dm !== undefined)
      setProbFavoriteDm(newSettings.prob_favorite_dm);
    if (newSettings.cross_universe_prob !== undefined)
      setCrossUniverseProb(newSettings.cross_universe_prob);
    if (newSettings.show_internal_thoughts !== undefined)
      setShowInternalThoughts(newSettings.show_internal_thoughts === 1);
    await apiFetch("/api/settings", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  const handleViewProfile = async (userId: number) => {
    ...
    const existingUser =
      ...
      exploreUsers.find((u) => u.id === userId);
    if (existingUser) {
  ...
  const handleViewUniverse = async (universeId: number) => {
    if (universeId === -1) {
      setViewingUniverse({
        id: -1,
        name: "None",
        description: "Characters without an assigned universe.",
      });
  ...
  const handleToggleUniversePause = async () => {
    ...
    setUniverses((prev) =>
      prev.map((u) => (u.id === viewingUniverse.id ? updatedUniverse : u)),
    );
    ...
    try {
      ...
      setUniverses((prev) =>
        prev.map((u) => (u.id === viewingUniverse.id ? viewingUniverse : u)),
      );
  ...
  const handleForcePost = async (type: "text" | "image", userId?: number) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (type === "image") {
          showToast(
            "Image post is generating in the background. It will appear shortly.",
          );
        } else {
  ...
  useEffect(() => {
    ...
    const isLastMsgProcessed = chatMessages.some(
      (m) => m.id === lastProcessedMsgId,
    );
    ...
    if (lastMsg.id === lastProcessedMsgId && !processingQueue.current) {
      ...
      const hasContentChanged = expanded.some(
        (m, i) =>
          m.content !== displayedMessages[i]?.content ||
          m.image_url !== displayedMessages[i]?.image_url ||
          m.image_request_status !==
            displayedMessages[i]?.image_request_status ||
          m.is_image_request !== displayedMessages[i]?.is_image_request,
      );
  ...
  useEffect(() => {
    ...
    const fetchTypingStatus = async () => {
      try {
        const res = await apiFetch(
          `/api/typing-status/${activeChat.id}?isGroup=${isGroupChat}`,
        );
        const data = await res.json();
        if (data && data.typing) {
  ...
  const handleToggleFavorite = async (targetId: number, isGroup: boolean) => {
    const isFav = dmFavorites.some(
      (f) => f.target_id === targetId && f.is_group === (isGroup ? 1 : 0),
    );
    try {
      if (isFav) {
        await apiFetch(`/api/favorites/${targetId}?is_group=${isGroup}`, {
          method: "DELETE",
        });
      } else {
  ...
  const handleOpenGallery = async () => {
    ...
    try {
      const endpoint = isGroupChat
        ? `/api/group-chats/${activeChat.id}/images`
        : `/api/dms/${activeChat.id}/images`;
      const res = await apiFetch(endpoint);
      if (res.ok) {
        const data = await res.json();
    ...
    try {
      await apiFetch("/api/posts", {
        ...
        body: JSON.stringify({
          ...
          post_type: type,
          image_url: image,
          universe_id: newPostUniverse ? parseInt(newPostUniverse) : null,
        }),
      });
      if (type === "image_post" && !image) {
        showToast(
          "Image is generating in the background. It will appear shortly.",
        );
      } else if (image) {
  ...
  const handleFollowAllInUniverse = async (chars: any[]) => {
    ...
    const updateFn = (prev: any[]) =>
      prev.map((u) => (idsToFollow.has(u.id) ? { ...u, is_followed: 1 } : u));
    setUsers(updateFn);
    setExploreUsers(updateFn);
    ...
    try {
      await Promise.all(
        charsToFollow.map((c) =>
          apiFetch(`/api/users/${c.id}/follow`, { method: "POST" }),
        ),
      );
    } catch (e) {
      fetchUsers();
  ...
  const handleUnfollowAllInUniverse = async (chars: any[]) => {
    ...
    const updateFn = (prev: any[]) =>
      prev.map((u) => (idsToUnfollow.has(u.id) ? { ...u, is_followed: 0 } : u));
    setUsers(updateFn);
    setExploreUsers(updateFn);

    try {
      await Promise.all(
        charsToUnfollow.map((c) =>
          apiFetch(`/api/users/${c.id}/follow`, { method: "POST" }),
        ),
      );
    } catch (e) {
  ...
  const handleToggleImageGen = async () => {
    ...
    try {
      const res = await apiFetch(
        `/api/dms/settings/${activeChat.id}?isGroup=${isGroupChat}`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const handleEditDm = async (msgId: number | string) => {
    ...
    const originalId =
      typeof msgId === "string" ? parseInt(msgId.split("_")[0]) : msgId;

    // Optimistic update
    const previousMessages = [...chatMessages];
    setChatMessages((prev) =>
      prev.map((m) =>
        m.id === msgId ? { ...m, content: editingDmContent.trim() } : m,
      ),
    );

    const endpoint = isGroupChat
      ? `/api/group-chats/messages/${originalId}`
      : `/api/dms/messages/${originalId}`;
    try {
      await apiFetch(endpoint, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const unreadMessages =
    ...
    groupChats.reduce((acc, curr) => acc + (curr.unread_count || 0), 0);
  ...
  const fetchPosts = useCallback(() => {
    ...
    const universeParam = timelineUniverseFilterRef.current
      ? `&universe_id=${timelineUniverseFilterRef.current}`
      : "";
  ...
  const fetchFauxPics = useCallback(() => {
    ...
    const universeParam = timelineUniverseFilterRef.current
      ...
      : "";
    apiFetch(
      `/api/posts?type=image_post&limit=${visibleFauxPicsRef.current + 1}${universeParam}`,
    )
      .then((r) => r.json())
      .then((data) => {
        setFauxPicsPosts(data);
        setIsFetchingFauxPics(false);
      })
  ...
  const fetchNews = useCallback(() => {
    ...
    const universeParam = timelineUniverseFilterRef.current
      ...
      : "";
    apiFetch(
      `/api/posts?account_type=news&limit=${visibleNewsRef.current + 1}${universeParam}`,
    )
      .then((r) => r.json())
      .then((data) => {
        setNewsPosts(data);
          ...
          <div className="absolute inset-[-50%] z-0 select-none flex justify-center items-center pointer-events-none opacity-20">
            <div className="w-[150vw] h-[150vh] flex flex-wrap gap-2 transform -rotate-12 justify-center items-center content-center relative">
              {Array.from({ length: 150 }).map((_, i) => {
                const avatarUrl =
                  loginBackgroundAvatars[i % loginBackgroundAvatars.length];
                return (
                  <div
                    key={i}
          ...
          <div className="flex flex-wrap justify-center gap-8 w-full">
            {realUsers.map((user) => (
              ...
              >
                ...
                <span
                  className={`text-xl font-medium transition-colors ${selectedLoginUser?.id === user.id ? "text-white" : "text-gray-400 group-hover:text-white"}`}
                >
                  {user.display_name}
                </span>
              </div>
              ...
              <form onSubmit={handleLogin} className="flex flex-col gap-4">
                {selectedLoginUser.has_pin && (
                  <div className="flex flex-col gap-2">
                    <label className="text-center text-gray-400 font-medium">
                      Enter PIN for {selectedLoginUser.display_name}
                    </label>
                    <input
                      type="password"
                      value={loginPin}
  ...
  return (
    <div className="min-h-screen bg-black text-white flex justify-center font-sans">
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-gray-900 border border-gray-800 rounded-2xl w-full max-w-2xl p-6 relative my-8">
            ...
            <h2 className="text-2xl font-bold mb-6 flex items-center gap-2">
              ...
              {editingArc ? "Edit" : "Create"}{" "}
              {(editingArc?.arc_type || isAddingArc?.type) === "universe"
                ? "Universe"
                : "Character"}{" "}
              Arc
            </h2>
            <form
              onSubmit={editingArc ? handleSaveArc : handleCreateArc}
              className="space-y-4"
            >
              <div>
              ...
              {((editingArc && editingArc.arc_type === "universe") ||
                (isAddingArc && isAddingArc.type === "universe")) && (
                <div>
                  <label className="block text-sm font-bold text-gray-400 mb-1 uppercase tracking-wider">
                    Current Status Text
                  </label>
                ...
                {editingArc ? (
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setArcDurationDays(parseInt(e.target.value) || 1)
                      }
                      min="1"
                      className="w-full bg-black border border-gray-800 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-orange-500 transition"
                    />
      ...
      <div className="flex h-screen w-full bg-slate-950 text-slate-100 overflow-hidden">
          ...
          <div>
            ...
            <nav className="space-y-2">
              ...
              <NavItem
                icon={<Camera />}
                label="FauxPics"
                active={activeTab === "fauxpics"}
                onClick={() => {
                  setActiveTab("fauxpics");
                  fetchFauxPics();
                }}
              />
              <NavItem
              ...
              <NavItem
                icon={<Calendar />}
                label="FauxPast"
                active={activeTab === "fauxpast"}
                onClick={() => setActiveTab("fauxpast")}
              />
              <NavItem
                icon={<Settings />}
                label="Settings"
              ...
              >
                ...
                <div className="hidden xl:block overflow-hidden">
                  <p className="font-bold text-sm truncate text-slate-200">
                    {loggedInUser?.display_name || "You"}
                  </p>
                  <p className="text-slate-500 text-xs truncate">
                    @{loggedInUser?.username || "real_user"}
                  </p>
                </div>
              </div>
              <button
        ...
        {/* Main Feed */}
        <div
          className={`flex-1 border-r border-white/10 relative custom-scrollbar pb-20 md:pb-0 ${activeTab === "messages" ? "flex flex-col overflow-hidden" : "overflow-y-auto"}`}
        >
          <div className="sticky top-0 bg-slate-950/80 backdrop-blur-xl border-b border-white/10 p-4 z-30 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <button
                className="md:hidden text-slate-100 p-1 -ml-1 hover:bg-white/10 rounded-lg transition"
                    ...
                    <div className="text-orange-500 flex gap-4 items-center">
                      <label className="cursor-pointer hover:bg-white/10 p-2 rounded-full transition">
                        ...
                        <input
                          ...
                          className="hidden"
                          accept="image/*"
                          onChange={(e) => handleFileUpload(e, setNewPostImage)}
                        />
                      </label>
                  ...
                  posts.length === 0 && (
                    <div className="p-8 text-center text-slate-500">
                      ...
                      <p className="text-sm mt-2">
                        Add characters to see them broadcast!
                      </p>
                    </div>
                  )
                )}
                ...
                {newsPosts.length === 0 && isFetchingNews ? (
                  ...
                  newsPosts.length === 0 && (
                    <div className="p-8 text-center text-slate-500">
                      ...
                      <p className="text-sm mt-2">
                        Follow more news accounts or wait for breaking news.
                      </p>
                    </div>
                  )
                )}
          ...
          {activeTab === "search" && (
            <div className="flex h-[calc(100vh-60px)]">
              <div className="flex-1 p-6 overflow-y-auto pb-24">
                <h2 className="text-2xl font-bold mb-6 tracking-tight text-slate-100">
                  Search Characters
                </h2>
                <div className="relative mb-6">
                  <input
                    type="text"
                    placeholder="Search characters..."
                    value={localExploreSearch}
                    onChange={(e) => setLocalExploreSearch(e.target.value)}
                    onKeyDown={(e) => {
                      if (e.key === "Enter") {
                        if (localExploreSearch !== characterSearch) {
                          setCharacterSearch(localExploreSearch);
                        }
                      }
                    }}
                    className="w-full bg-slate-900 border border-white/10 text-white px-4 py-3 pr-12 rounded-xl outline-none focus:border-orange-500"
                  />
                  <button
                    onClick={() => {
                      if (localExploreSearch !== characterSearch) {
                ...
                >
                  ...
                  {charAccountType === "character" ? (
                    <>
                      ...
                      <div>
                        <label className="block text-sm font-medium text-slate-400 mb-1">
                          Additional Info (Franchise, Context, etc.)
                        </label>
                        <textarea
                          value={charPersona}
                          ...
                          rows={2}
                          className="w-full bg-slate-900 border border-white/10 rounded-xl p-3 text-slate-100 outline-none focus:border-orange-500 transition"
                          placeholder="e.g. From The Witcher 3, currently looking for Ciri..."
                        ></textarea>
                      </div>
                      <div>
                        <label className="flex items-center gap-2 cursor-pointer mt-2">
                          <input
                            ...
                            onChange={(e) =>
                              setCharIsVerified(e.target.checked)
                            }
                          ...
                          <span className="text-sm font-medium text-slate-400 group-hover:text-slate-200 transition-colors">
                            Verified Public Figure / Celebrity
                          </span>
                        </label>
                      </div>
                    </>
                    ...
                    <>
                      <div>
                        ...
                        <input
                          required
                          value={charCompanyName}
                          onChange={(e) => {
                            setCharCompanyName(e.target.value);
                      ...
                      <div>
                        ...
                        <textarea
                          value={charBrandIdentity}
                          onChange={(e) => setCharBrandIdentity(e.target.value)}
                          rows={2}
                          className="w-full bg-slate-900 border border-white/10 rounded-xl p-3 text-slate-100 outline-none focus:border-orange-500 transition"
                          placeholder="e.g. Corporate, patriotic, slightly sinister..."
                        ></textarea>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-slate-400 mb-1">
                        ...
                        <textarea
                          value={charProductsServices}
                          onChange={(e) =>
                            setCharProductsServices(e.target.value)
                          }
                          rows={2}
                          className="w-full bg-slate-900 border border-white/10 rounded-xl p-3 text-slate-100 outline-none focus:border-orange-500 transition"
                          placeholder="e.g. Compound V, Superheroes, Energy Drinks..."
                        ></textarea>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-slate-400 mb-1">
                        ...
                        <input
                          value={charTargetAudience}
                          onChange={(e) =>
                            setCharTargetAudience(e.target.value)
                          }
                          type="text"
                          className="w-full bg-slate-900 border border-white/10 rounded-xl p-3 text-slate-100 outline-none focus:border-orange-500 transition"
                          placeholder="e.g. General public, superhero fans"
                        />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-slate-400 mb-1">
                          Run By Character (Optional)
                        </label>
                        <select
                          value={charRunByCharacterId || ""}
                          onChange={(e) =>
                            setCharRunByCharacterId(
                    ...
                    <>
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          rows={2}
                          className="w-full bg-slate-900 border border-white/10 rounded-xl p-3 text-slate-100 outline-none focus:border-orange-500 transition"
                          placeholder="e.g. The premier news source for Metropolis..."
                        ></textarea>
                      </div>
                    </>
                  ) : null}
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        required
                        value={charUsername}
                        onChange={(e) => setCharUsername(e.target.value)}
                        type="text"
                        className="w-full bg-slate-900 border border-white/10 rounded-xl p-3 text-slate-100 outline-none focus:border-orange-500 transition"
                        placeholder="e.g. white_wolf"
                      />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-slate-400 mb-1">
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        type="text"
                        className="flex-1 bg-slate-900 border border-white/10 rounded-xl p-3 text-slate-100 outline-none focus:border-orange-500 transition"
                      ...
                      <label className="bg-slate-800 hover:bg-slate-700 text-white px-4 py-3 rounded-xl cursor-pointer flex items-center gap-2 transition">
                        ...
                        <input
                          ...
                          className="hidden"
                          accept="image/*"
                          onChange={(e) => handleFileUpload(e, setCharAvatar)}
                        />
                      </label>
                    </div>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-400 mb-1">
                      Reference Images (Overrides Profile Pic for Image Gen)
                    </label>
                    <div className="flex flex-col gap-2">
                      {charReferenceImages.map((img, idx) => (
                        <div key={idx} className="flex gap-2 items-center">
                          ...
                          <input
                            ...
                            type="text"
                            className="flex-1 bg-slate-900 border border-white/10 rounded-xl p-2 text-slate-100 outline-none focus:border-orange-500 transition"
                            placeholder="Image URL..."
                          />
                          <button
                            type="button"
                            onClick={() =>
                              setCharReferenceImages(
                                charReferenceImages.filter((_, i) => i !== idx),
                              )
                            }
                      ...
                      <div className="flex gap-2">
                        <button
                          type="button"
                          onClick={() =>
                            setCharReferenceImages([...charReferenceImages, ""])
                          }
                          className="bg-slate-800 hover:bg-slate-700 text-white px-4 py-2 rounded-xl flex items-center gap-2 text-sm transition"
                        >
                  ...
                  {charAccountType !== "news" && (
                    <div>
                      ...
                      <textarea
                        ...
                        rows={3}
                        className="w-full bg-slate-900 border border-white/10 rounded-xl p-3 text-slate-100 outline-none focus:border-orange-500 transition"
                        placeholder="Detailed personality and background..."
                      ></textarea>
                    </div>
                  )}
                  <div>
                    <label className="block text-sm font-medium text-slate-400 mb-1">
                      Writing Style (Private)
                    </label>
                    <textarea
                      value={charWritingStyle}
                      onChange={(e) => setCharWritingStyle(e.target.value)}
                      rows={3}
                      className="w-full bg-slate-900 border border-white/10 rounded-xl p-3 text-slate-100 outline-none focus:border-orange-500 transition"
                      placeholder="Tone of voice, catchphrases, interaction style..."
                    ></textarea>
                  </div>
                  {charAccountType === "character" && (
                    <>
                      <div>
                        <label className="block text-sm font-medium text-slate-400 mb-1">
                          Physical Appearance (Private)
                        </label>
                        <textarea
                          value={charPhysicalAppearance}
                          onChange={(e) =>
                            setCharPhysicalAppearance(e.target.value)
                          }
                          rows={2}
                          className="w-full bg-slate-900 border border-white/10 rounded-xl p-3 text-slate-100 outline-none focus:border-orange-500 transition"
                          placeholder="Hair color, body type, facial features..."
                        ></textarea>
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-slate-400 mb-1">
                          Clothing Style (Private)
                        </label>
                        <textarea
                          ...
                          rows={2}
                          className="w-full bg-slate-900 border border-white/10 rounded-xl p-3 text-slate-100 outline-none focus:border-orange-500 transition"
                          placeholder="Usual outfits, fashion sense, accessories..."
                        ></textarea>
                      </div>
                    </>
                  )}
                  {charAccountType !== "news" && (
                    <div>
                      ...
                      <textarea
                        value={charArtstyle}
                        onChange={(e) => setCharArtstyle(e.target.value)}
                        rows={2}
                        className="w-full bg-slate-900 border border-white/10 rounded-xl p-3 text-slate-100 outline-none focus:border-orange-500 transition"
                        placeholder="e.g. Anime, Realistic, Pixel Art, Oil Painting..."
                      ></textarea>
                    </div>
                  )}
                  <div>
                    <label className="block text-sm font-medium text-slate-400 mb-2">
                      Online Time (Optional - Default: Always Online)
                    </label>
                      ...
                      {ONLINE_TIME_WINDOWS.map((window) => (
                        ...
                        >
                          <input
                            ...
                            onChange={(e) => {
                              if (e.target.checked) {
                                setCharOnlineTimes((prev) => [
                                  ...prev,
                                  window.value,
                                ]);
                              } else {
                                setCharOnlineTimes((prev) =>
                                  prev.filter((t) => t !== window.value),
                                );
                              }
                            }}
                    ...
                    <p className="text-xs text-slate-500 mt-2 italic">
                      If no window is selected, the character is online 24/7.
                    </p>
                  </div>
                  {charAccountType !== "news" && (
                    <div>
                      ...
                      <div className="flex items-center gap-4">
                        <input
                          ...
                          onChange={(e) =>
                            setCharActivityLevel(parseInt(e.target.value))
                          }
                      ...
                      <p className="text-xs text-slate-500 mt-1">
                        Dictates how often this character creates posts,
                        comments, and DMs.
                      </p>
                    </div>
                  )}
                  <button
                    type="submit"
                    className="w-full bg-gradient-to-r from-orange-500 to-orange-600 text-white font-bold py-3 rounded-xl hover:from-orange-600 hover:to-orange-700 transition shadow-[0_0_15px_rgba(249,115,22,0.4)]"
                  >
                    Add Character
                  </button>
                </form>
              ...
              {!activeChat && (
                <div className="w-full flex-1 flex flex-col min-h-0">
                  <div className="p-4 border-b border-white/10 flex justify-between items-center bg-slate-950/80 backdrop-blur-xl">
                    <h2 className="font-bold text-lg tracking-tight text-slate-100">
                      Comms
                    </h2>
                    <button
                  ...
                  <div className="overflow-y-auto flex-1 divide-y divide-white/5">
                    {groupChats.map((group) => (
                      <div
                        ...
                        onClick={() => {
                          const isAlreadyOpen =
                            activeChat?.id === group.id && isGroupChat;
                          setActiveChat({
                            id: group.id,
                            name: group.name,
                            account_type: "group",
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className="relative w-12 h-12 flex-shrink-0 cursor-pointer">
                            ...
                            {group.members?.some(
                              (m: any) => m.is_ai === 1 && isUserOnline(m),
                            ) && (
                              <div
                                className="absolute bottom-0 right-0 w-3.5 h-3.5 rounded-full border-2 border-slate-950 bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]"
                                title="AI Member Online"
                              ></div>
                            )}
                          </div>
                          <div className="overflow-hidden flex-1">
                            ...
                            <p
                              className={`text-sm truncate ${group.unread_count > 0 ? "text-white font-bold" : "text-slate-500"}`}
                            >
                    ...
                    {conversations.map((conv) => (
                      <div
                        ...
                        onClick={() => {
                          const isAlreadyOpen =
                            activeChat?.id === conv.other_user_id &&
                            !isGroupChat;
                          setActiveChat({
                            id: conv.other_user_id,
                            name: conv.display_name,
                            avatar_url: conv.avatar_url,
                            account_type: conv.account_type,
                            isGroup: false,
                          });
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className="relative w-12 h-12 flex-shrink-0 cursor-pointer">
                            ...
                            {conv.is_ai === 1 && (
                              <div
                                className={`absolute bottom-0 right-0 w-3.5 h-3.5 rounded-full border-2 border-slate-950 ${isUserOnline(conv) ? "bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]" : "bg-slate-500"}`}
                                title={
                                  isUserOnline(conv) ? "Online" : "Offline"
                                }
                              ></div>
                            )}
                          </div>
                          <div className="overflow-hidden flex-1">
                            ...
                            <p
                              className={`text-sm truncate ${conv.unread_count > 0 ? "text-white font-bold" : "text-slate-500"}`}
                            >
                              {conv.last_message}
                            </p>
                          </div>
                ...
                <div className="w-full flex-1 flex flex-col min-h-0">
                  <div className="p-4 border-b border-white/10 font-bold flex items-center justify-between bg-slate-950/80 backdrop-blur-xl shrink-0">
                        ...
                        {(() => {
                          if (isGroupChat) {
                            ...
                            if (
                              ...
                              return (
                                <div
                                  className="absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full border-2 border-slate-950 bg-green-500 shadow-[0_0_6px_rgba(34,197,94,0.6)]"
                                  title="AI Member Online"
                                ></div>
                              );
                            }
                            ...
                            const user = users.find(
                              (u) => u.id === activeChat.id,
                            );
                            if (user?.is_ai === 1) {
                              return (
                                <div
                                  className={`absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full border-2 border-slate-950 ${isUserOnline(user) ? "bg-green-500 shadow-[0_0_6px_rgba(34,197,94,0.6)]" : "bg-slate-500"}`}
                                  title={
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <button
                        onClick={() =>
                          handleToggleFavorite(activeChat.id, isGroupChat)
                        }
                        ...
                        title={
                          dmFavorites.some(
                            (f) =>
                              f.target_id === activeChat.id &&
                              f.is_group === (isGroupChat ? 1 : 0),
                          )
                            ? "Unfavorite"
                            : "Favorite"
                        }
                    ...
                    {isFetchingChatMessages ? (
                      ...
                      <>
                        ...
                        {displayedMessages.map((msg, i) => {
                          ...
                          const prevMsg =
                            i > 0 ? displayedMessages[i - 1] : null;
                          ...
                          return (
                            <React.Fragment key={i}>
                              ...
                              <div
                                className={`flex flex-col ${isMe ? "items-end" : "items-start"} ${isLastInSequence ? "mb-2" : "mb-0.5"} w-full`}
                              >
                                {!isMe &&
                                  isGroupChat &&
                                  sender &&
                                  isFirstInSequence && (
                                    <span className="text-xs text-slate-400 ml-9 mb-1">
                                      {sender.display_name}
                                    </span>
                                  )}
                                <div
                                  className={`flex w-full ${isMe ? "justify-end" : "justify-start"} gap-2 items-end`}
                                >
                                  {!isMe && (
                                      ...
                                      {isLastInSequence ? (
                                        <div className="relative">
                                          <img loading="lazy" decoding="async"
                                            ...
                                            alt=""
                                            className={`w-6 h-6 ${getAvatarShape(sender?.account_type || activeChat.account_type)} object-cover flex-shrink-0 mb-1 border border-white/10`}
                                          />
                                          {sender?.is_ai === 1 && (
                                            <div
                                              className={`absolute bottom-1 -right-0.5 w-2 h-2 rounded-full border border-slate-950 ${isUserOnline(sender) ? "bg-green-500" : "bg-slate-500"}`}
                                              title={
                                                isUserOnline(sender)
                                                  ? "Online"
                                                  : "Offline"
                                  ...
                                  <div
                                    className={`group flex flex-col max-w-[75%] ${isMe ? "items-end" : "items-start"}`}
                                  >
                                    <div
                                      className={`rounded-2xl p-3 whitespace-pre-wrap break-words border ${isMe ? "bg-orange-500/20 border-orange-500/30 text-slate-100 rounded-br-none" : "bg-slate-800/50 border-white/10 text-slate-100 rounded-bl-none backdrop-blur-sm"}`}
                                    >
                                      {msg.image_url && (
                                        <div
                                          className="mb-2 rounded-lg overflow-hidden border border-white/10 cursor-pointer"
                                          onClick={() =>
                                      ...
                                      {editingDmId === msg.id ? (
                                        <div className="flex flex-col gap-2 min-w-[200px]">
                                          ...
                                          <div className="flex justify-end gap-2">
                                            ...
                                            <button
                                              ...
                                              className={`text-xs px-2 py-1 rounded ${isMe ? "bg-white text-orange-600 hover:bg-slate-100" : "bg-orange-600 text-white hover:bg-orange-500"}`}
                                            >
                                              Save
                                            </button>
                                          </div>
                                        ...
                                        <>
                                          ...
                                          {msg.internal_thought && (
                                            <div
                                              className={`mt-2 p-2 rounded-lg text-xs italic relative overflow-hidden group ${isMe ? "bg-orange-600/50 border-l-2 border-white/50 text-white/90" : "bg-slate-700/50 border-l-2 border-orange-500 text-slate-300"}`}
                                            >
                                              <div className="absolute -right-2 -top-2 opacity-5">
                                                <Brain size={32} />
                                              </div>
                                              <div
                                                className={`flex items-center gap-1 mb-1 font-bold text-[10px] uppercase tracking-wider ${isMe ? "text-white/70" : "text-orange-400/80"}`}
                                              >
                                                <Brain size={10} />
                                                <span>Thought</span>
                                              </div>
                                          ...
                                          {msg.is_image_request === 1 &&
                                            !String(msg.id).includes(
                                              "_image",
                                            ) && (
                                              <div
                                                className={`mt-3 p-3 rounded-xl border flex flex-col gap-2 ${isMe ? "bg-orange-800/40 border-orange-400/30" : "bg-slate-900/60 border-slate-600/50"}`}
                                              >
                                                <div className="flex items-center gap-2">
                                                  ...
                                                  />
                                                  <p
                                                    className={`text-xs font-semibold ${isMe ? "text-orange-100" : "text-slate-200"}`}
                                                  >
                                                    {sender?.display_name ||
                                                      activeChat.name}{" "}
                                                  ...
                                                  <div className="flex gap-2 w-full mt-1">
                                                    <button
                                                      onClick={() => {
                                                        ...
                                                        apiFetch(
                                                          `/api/dms/messages/${msg.id}/accept-image`,
                                                          { method: "POST" },
                                                        ).then(() =>
                                                          fetchChatMessages(
                                                            activeChat.id,
                                                    ...
                                                    <button
                                                      onClick={() => {
                                                        ...
                                                        apiFetch(
                                                          `/api/dms/messages/${msg.id}/decline-image`,
                                                          { method: "POST" },
                                                        ).then(() =>
                                                          fetchChatMessages(
                                                            activeChat.id,
                                                ...
                                                {msg.image_request_status ===
                                                  ...
                                                  <div className="flex items-center gap-2 py-1">
                                                    ...
                                                    />
                                                    <span className="text-[10px] uppercase tracking-wider font-bold text-orange-400/80">
                                                      Generating Image...
                                                    </span>
                                                  </div>
                                                )}
                                                ...
                                                {msg.image_request_status ===
                                                  "accepted" && (
                                                  <div className="py-1">
                                                    <span className="text-[10px] uppercase tracking-wider font-bold text-green-400/80">
                                                      Request Accepted
                                                    </span>
                                                  </div>
                                                )}
                                                ...
                                                {msg.image_request_status ===
                                                  "declined" && (
                                                  <div className="py-1">
                                                    <span className="text-[10px] uppercase tracking-wider font-bold text-slate-500/80">
                                                      Request Declined
                                                    </span>
                                                  </div>
                                                )}
                                    ...
                                    <div
                                      className={`flex gap-2 mt-1 opacity-0 group-hover:opacity-100 transition-opacity ${isMe ? "justify-end" : "justify-start"}`}
                                    >
                                      <button
                                        onClick={() => {
                                          const originalId =
                                            typeof msg.id === "string"
                                              ? parseInt(msg.id.split("_")[0])
                                              : msg.id;
                        ...
                        {(typingUser || serverTypingUsers.length > 0) && (
                          <div className="flex flex-col gap-2 mb-4">
                            {serverTypingUsers
                              .filter((u) => u !== typingUser)
                              .map((u) => (
                                <div
                                  key={u}
                                  className="flex items-center gap-2 text-slate-400 text-xs italic ml-9"
                                >
          ...
          {activeTab === "universes" && (
            <div className="p-6 max-w-4xl mx-auto">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <div
                  onClick={() => handleViewUniverse(-1)}
                  className="bg-slate-900/50 border border-white/10 rounded-2xl p-4 cursor-pointer hover:bg-slate-800/50 hover:border-orange-500/30 transition flex flex-col h-full backdrop-blur-sm group"
                >
                  <div className="flex items-center gap-4 mb-3">
                    <div className="w-16 h-16 bg-slate-800 rounded-full overflow-hidden flex-shrink-0 border border-white/10 group-hover:border-orange-500/50 transition-colors">
                      <Globe
                        size={32}
                        className="m-auto mt-4 text-slate-500 group-hover:text-orange-400 transition-colors"
                      />
                    </div>
                    <div>
                    ...
                    {(universeCharactersMap["none"] || [])
                      ...
                      .map((char) => (
                        ...
                        >
                          ...
                          />
                          {char.is_active &&
                            isUserOnline(char, currentTimeInMinutes) && (
                ...
                {universes.map((u) => (
                  <div
                    key={u.id}
                    onClick={() => handleViewUniverse(u.id)}
                    className={`bg-slate-900/50 border border-white/10 rounded-2xl p-4 cursor-pointer hover:bg-slate-800/50 hover:border-orange-500/30 transition flex flex-col h-full backdrop-blur-sm group ${u.is_paused ? "opacity-50 grayscale" : ""}`}
                  >
                    <div className="flex items-center gap-4 mb-3">
                      <div className="w-16 h-16 bg-slate-800 rounded-full overflow-hidden flex-shrink-0 border border-white/10 group-hover:border-orange-500/50 transition-colors">
                        {u.image_url ? (
                      ...
                      <div>
                        <h3 className="font-bold text-lg text-slate-100 group-hover:text-orange-400 transition-colors">
                          {u.name}
                          {u.is_paused === 1 && (
                            <span className="ml-2 text-[10px] uppercase tracking-wider font-bold bg-slate-800 text-slate-400 px-2 py-0.5 rounded border border-white/10">
                              Paused
                            </span>
                          )}
                        </h3>
                        <p className="text-sm text-slate-400">
                          {universeCounts[u.id] || 0} characters
                        </p>
                      </div>
                    </div>
                    {u.description && (
                ...
                {universes.length === 0 && (
                  <div className="col-span-full p-8 text-center text-slate-500">
                    <p>
                      No universes created yet. You can create one when adding
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setEditUniverseImageUrl(e.target.value)
                        }
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          setEditUniverseDescription(e.target.value)
                        }
                  ...
                  <div className="flex flex-col md:flex-row gap-6 items-start">
                    <div className="w-32 h-32 bg-slate-800 rounded-full overflow-hidden flex-shrink-0 border border-white/10">
                      {viewingUniverse.image_url ? (
                        ...
                        <Globe
                          size={64}
                          className="m-auto mt-8 text-slate-500"
                        />
                      )}
                    </div>
                    <div>
                      <h2 className="text-3xl font-bold mb-2 text-slate-100 flex items-center gap-3 flex-wrap">
                        ...
                        {viewingUniverse.is_paused === 1 && (
                          <span className="text-xs uppercase tracking-wider font-bold bg-slate-800 text-slate-400 px-3 py-1 rounded-lg border border-white/10">
                            Paused
                          </span>
                        )}
                      </h2>
                      <p className="text-slate-400 mb-4">
                        {viewingUniverseCharacters.length} characters
                      </p>
              ...
              {universeActiveTab === "characters" &&
                viewingUniverseCharacters.length > 0 && (
                  <div className="flex gap-2 mb-4">
                    <button
                      onClick={() =>
                        handleFollowAllInUniverse(viewingUniverseCharacters)
                      }
                    ...
                    <button
                      onClick={() =>
                        handleUnfollowAllInUniverse(viewingUniverseCharacters)
                      }
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                        ...
                        >
                          <div className="relative">
                            <img loading="lazy" decoding="async"
                              ...
                              alt={char.display_name}
                              className={`w-12 h-12 ${getAvatarShape(char.account_type)} object-cover border border-white/10 group-hover:border-orange-500/50 transition-colors`}
                              referrerPolicy="no-referrer"
                            />
                            {char.is_active && isUserOnline(char) && (
                              <div
                                className="absolute bottom-0 right-0 w-3 h-3 bg-green-500 border-2 border-slate-950 rounded-full shadow-[0_0_6px_rgba(34,197,94,0.6)]"
                                title="Online"
                              ></div>
                            )}
                          </div>
                          <div className="overflow-hidden flex-1">
                            <p className="font-bold truncate flex items-center gap-1 text-slate-100 group-hover:text-orange-400 transition-colors flex-wrap">
                              ...
                              {!char.is_active && (
                                <span className="text-[10px] bg-slate-800 px-2 py-0.5 rounded text-slate-400 border border-white/10">
                                  Inactive
                                </span>
                              )}
                            </p>
                      ...
                      {viewingUniverseCharacters.length === 0 && (
                        <p className="text-slate-500 col-span-full">
                          No characters found in this universe.
                        </p>
                      )}
                    </div>
                  ) : (
                    <div className="space-y-4">
                      {loggedInUser?.role === "admin" && (
                        <div className="flex gap-2 mb-4">
                          <button
                            onClick={() =>
                              handleAddArc("universe", viewingUniverse.id)
                            }
                            disabled={viewingUniverse.is_paused}
                            title={
                              viewingUniverse.is_paused
                                ? "Cannot add arcs to a paused universe"
                                : ""
                            }
                          ...
                          <button
                            onClick={() =>
                              handleGenerateArc("universe", viewingUniverse.id)
                            }
                            disabled={
                              isGeneratingArc || viewingUniverse.is_paused
                            }
                            title={
                              viewingUniverse.is_paused
                                ? "Cannot generate arcs for a paused universe"
                                : ""
                            }
                          ...
                          >
                            <div className="flex justify-between items-start mb-4">
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                <span
                                  className={`text-xs font-bold px-3 py-1 rounded-full ${arc.status === "active" ? "bg-green-500/20 text-green-500 border border-green-500/30" : "bg-slate-800 text-slate-400 border border-white/10"}`}
                                >
                            ...
                            <div className="mb-4 p-4 bg-slate-800/50 rounded-lg border border-white/10">
                              ...
                              <p className="text-slate-200 text-sm whitespace-pre-wrap">
                                {arc.current_status_text}
                              </p>
                            </div>

                            {arc.status === "completed" &&
                              arc.completion_summary && (
                                <div className="mb-4 p-4 bg-orange-500/10 rounded-lg border border-orange-500/20">
                                  ...
                                  <p className="text-orange-200 text-sm whitespace-pre-wrap">
                                    {arc.completion_summary}
                                  </p>
                                </div>
                              )}
                            ...
                            <div className="flex gap-6 text-xs text-slate-500 border-t border-white/10 pt-4 mt-4">
                              <span>
                                Started:{" "}
                                {new Date(arc.start_date).toLocaleDateString()}
                              </span>
                              <span>
                                Target End:{" "}
                                {new Date(
            ...
            <div className="p-6 max-w-4xl mx-auto">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {followedUsers.map((char) => (
                  <div
                    key={char.id}
                    className={`bg-slate-900/50 border border-white/10 rounded-xl p-4 flex items-center justify-between gap-3 hover:bg-slate-800/50 hover:border-orange-500/30 transition backdrop-blur-sm group ${!char.is_active ? "opacity-50 grayscale" : ""}`}
                  >
                    <div
          ...
          {activeTab === "arcs" && (
            <div className="p-6 max-w-4xl mx-auto">
              ...
              <p className="text-slate-400 mb-6">
                This tab shows all active and recently completed arcs for
              ...
              <div className="space-y-4">
                {arcs.length === 0 ? (
                  ...
                  arcs.map((arc: any) => (
                    <div
                      key={`${arc.arc_type}-${arc.id}`}
                      className="bg-slate-900/50 border border-white/10 rounded-2xl p-5 flex flex-col gap-4 relative group backdrop-blur-sm"
                    >
                      {loggedInUser?.role === "admin" && (
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-slate-500">
                              ...
                              •{" "}
                              {new Date(
                                arc.last_update_date || arc.created_at,
                              ).toLocaleString()}
                            </p>
                          </div>
                        </div>
                        <div
                          className={`px-3 py-1 rounded-full text-xs font-bold ${arc.status === "completed" ? "bg-green-500/20 text-green-400 border border-green-500/30" : "bg-blue-500/20 text-blue-400 border border-blue-500/30"}`}
                        >
          ...
          {activeTab === "relationships" && loggedInUser?.role === "admin" && (
            <div className="p-6 max-w-4xl mx-auto">
              ...
              <p className="text-slate-400 mb-6">
                This tab shows all the times the AI evaluated whether two
                characters formed a meaningful relationship based on their
                interactions.
              </p>
              ...
              <div className="space-y-4">
                {relationshipChecks.length === 0 ? (
                  <div className="text-center py-12 text-slate-500">
                    ...
                    <p className="text-sm mt-2">
                      Characters need to interact more (comments or DMs) to
                      trigger a check.
                    </p>
                  </div>
                ) : (
                  relationshipChecks.map((check: any) => (
                    <div
                      key={check.id}
                      className="bg-slate-900/50 border border-white/10 rounded-2xl p-5 flex flex-col gap-4 backdrop-blur-sm"
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                          <div className="flex -space-x-4">
                            ...
                            <img loading="lazy" decoding="async"
                              ...
                              alt={check.user2_name}
                              className={`w-12 h-12 ${getAvatarShape(check.user2_account_type)} border-2 border-slate-950 object-cover bg-slate-800`}
                              referrerPolicy="no-referrer"
                            />
                          </div>
                          <div>
                            <h3 className="font-bold text-lg text-slate-100">
                              {check.user1_name} & {check.user2_name}
                            </h3>
                            <p className="text-xs text-slate-500">
                        ...
                        <div
                          className={`px-3 py-1 rounded-full text-xs font-bold ${check.result ? "bg-green-500/20 text-green-400 border border-green-500/30" : "bg-red-500/20 text-red-400 border border-red-500/30"}`}
                        >
                          {check.result
                            ? check.is_update
                      ...
                      <div className="bg-slate-950/50 rounded-xl p-4 text-sm border border-white/5">
                        <div className="flex items-center gap-2 mb-2 text-slate-400">
                          <span className="font-mono text-xs bg-slate-800 px-2 py-0.5 rounded border border-white/10">
                            Threshold: {check.interaction_threshold}
                          </span>
                          {check.is_update ? (
                            <span className="font-mono text-xs bg-blue-500/20 text-blue-400 px-2 py-0.5 rounded border border-blue-500/30">
                              Update Check
                            </span>
                          ) : (
                            <span className="font-mono text-xs bg-purple-500/20 text-purple-400 px-2 py-0.5 rounded border border-purple-500/30">
                              New Check
                            </span>
                          )}
                        </div>
                        {check.description ? (
                          ...
                          <p className="text-slate-500 italic">
                            The AI determined their interactions were not
                            significant enough to{" "}
                            {check.is_update
                              ? "update their existing relationship"
                              : "form a hard-coded relationship"}
                            .
                          </p>
                        )}
                      </div>
      ...
      .then((data) => { if (Array.isArray(data)) setRealUsers(data); });
                      ...
                      <p className="text-xs text-slate-500 mt-2">
                        Leave empty to remove PIN. Only 4-digit numeric PINs are
                        supported.
                      </p>
                    </div>
                  </div>
                ...
                {loggedInUser?.role === "admin" && (
                  <section className="bg-slate-900/50 border border-white/10 p-6 rounded-2xl backdrop-blur-sm">
                    ...
                    <p className="text-sm text-slate-400 mb-4">
                      Add new real users to the platform. They will have their
                      own profile, timeline, and messages.
                    </p>
      ...
      .then((data) => { if (Array.isArray(data)) setRealUsers(data); });
                    ...
                    >
                      ...
                      <div>
                        ...
                        <input
                          ...
                          required
                          type="text"
                          className="w-full bg-slate-900 border border-white/10 rounded-lg p-2 text-slate-100 outline-none focus:border-orange-500 text-sm placeholder-slate-500"
                        />
                      </div>
                      <div>
                        <label className="block text-xs font-medium text-slate-400 mb-1">
                      ...
                      <div>
                        <label className="block text-xs font-medium text-slate-400 mb-1">
                          PIN (Optional, 4 digits recommended)
                        </label>
                        <input
                          name="pin"
                          type="password"
                    ...
                    <div className="mt-6">
                      <h4 className="font-bold text-sm mb-2 text-slate-100">
                        Existing Real Users
                      </h4>
                      <div className="space-y-2">
                        {realUsers.map((u) => (
                          <div
                            key={u.id}
                            className="flex items-center justify-between bg-slate-950/50 p-3 rounded-lg border border-white/10"
                          >
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <div className="font-bold text-sm text-slate-100">
                                  {u.display_name}{" "}
                                  {u.role === "admin" && (
                                    <span className="text-[10px] bg-orange-500 text-white px-1.5 py-0.5 rounded ml-1">
                                      ADMIN
                                    </span>
                                  )}
                                </div>
                  ...
                  <section className="bg-slate-900/50 border border-white/10 p-6 rounded-2xl backdrop-blur-sm">
                    ...
                    <div className="flex items-center justify-between mb-6">
                      <div>
                        <p className="font-medium text-slate-100">
                          Enable AI Background Worker
                        </p>
                        <p className="text-sm text-slate-500 mt-1">
                          When enabled, AI characters will automatically post,
                          comment, and send DMs.
                        </p>
                      </div>
                      <button
                      ...
                      >
                        <div
                          className={`w-6 h-6 bg-white rounded-full shadow-md transform transition-transform duration-200 ease-in-out ${aiEnabled ? "translate-x-6" : "translate-x-0"}`}
                        />
                      </button>
                    </div>
                    ...
                    <div className="flex items-center justify-between mb-6">
                      <div>
                        ...
                        <p className="text-sm text-slate-500 mt-1">
                          When enabled, AI characters may generate explicit
                      ...
                      >
                        <div
                          className={`w-6 h-6 bg-white rounded-full shadow-md transform transition-transform duration-200 ease-in-out ${allowNsfw ? "translate-x-6" : "translate-x-0"}`}
                        />
                      </button>
                    </div>
                    ...
                    <div className="flex items-center justify-between mb-6">
                      <div>
                        ...
                        <p className="text-sm text-slate-500 mt-1">
                          When enabled, a detailed log file will be generated in
                          the backend to help diagnose performance issues.
                        </p>
                      </div>
                      <button
                      ...
                      >
                        <div
                          className={`w-6 h-6 bg-white rounded-full shadow-md transform transition-transform duration-200 ease-in-out ${enablePerformanceLogging ? "translate-x-6" : "translate-x-0"}`}
                        />
                      </button>
                    </div>
                    ...
                    <div className="mb-6">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          onClick={saveApiKey}
                          className="bg-orange-600 hover:bg-orange-500 text-white font-bold py-2 px-4 rounded-lg transition"
                        >
                          Save
                        </button>
                      </div>
                    ...
                    <div className="mb-6">
                      <h4 className="text-md font-bold mb-2 text-slate-100">
                        AI Activity Probabilities (per Day)
                      </h4>
                      <p className="text-sm text-slate-400 mb-4">
                        Adjust how often AI characters perform actions on
                        average per day.
                      </p>
                      ...
                      <div className="space-y-4">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              handleUpdateSettings({
                                prob_post: parseInt(e.target.value),
                              })
                            }
                            className="w-full accent-orange-500"
                          />
                        ...
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              handleUpdateSettings({
                                prob_comment: parseInt(e.target.value),
                              })
                            }
                            className="w-full accent-orange-500"
                          />
                        ...
                        <div>
                          <label className="block text-sm font-medium text-slate-400 mb-1">
                            Direct Messages ({probMessage}/day)
                          </label>
                          <input
                            type="range"
                            min="0"
                            ...
                            onChange={(e) =>
                              handleUpdateSettings({
                                prob_message: parseInt(e.target.value),
                              })
                            }
                            className="w-full accent-orange-500"
                          />
                        ...
                        <div>
                          <label className="block text-sm font-medium text-slate-400 mb-1">
                            Favorite DM Chance ({probFavoriteDm}%)
                          </label>
                          <p className="text-xs text-slate-500 mb-2">
                            Probability that a DM will be sent to a favorited
                            conversation vs a random character.
                          </p>
                          <input
                            type="range"
                            min="0"
                            max="100"
                            value={probFavoriteDm}
                            onChange={(e) =>
                              handleUpdateSettings({
                                prob_favorite_dm: parseInt(e.target.value),
                              })
                            }
                            className="w-full accent-orange-500"
                          />
                        ...
                        <div>
                          <label className="block text-sm font-medium text-slate-400 mb-1">
                            Cross-Universe Interaction ({crossUniverseProb}%)
                          </label>
                          <p className="text-xs text-slate-500 mb-2">
                            Probability that a character will interact with
                            someone from a different universe.
                          </p>
                          <input
                            type="range"
                            min="0"
                            max="100"
                            value={crossUniverseProb}
                            onChange={(e) =>
                              handleUpdateSettings({
                                cross_universe_prob: parseInt(e.target.value),
                              })
                            }
                            className="w-full accent-orange-500"
                          />
                        ...
                        <div className="flex items-center justify-between p-3 bg-slate-950/50 rounded-lg border border-white/10">
                          <div>
                            ...
                            <p className="text-xs text-slate-500">
                              Reveal the private thoughts of AI characters on
                              their posts and messages.
                            </p>
                          </div>
                          <button
                            onClick={() =>
                          ...
                          >
                            <div
                              className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-all ${showInternalThoughts ? "left-7" : "left-1"}`}
                            />
                          </button>
                        </div>
                      </div>
                    ...
                    <div className="mb-6">
                      <h4 className="text-md font-bold mb-2 text-slate-100">
                        Post Archetype Probabilities
                      </h4>
                      <p className="text-sm text-slate-400 mb-4">
                        Adjust the relative likelihood of each post type when an
                        AI decides to post.
                      </p>
                      <div className="space-y-4">
                        {archetypes.map((arch, index) => (
                          <div key={arch.id}>
                            <label className="block text-sm font-medium text-slate-400 mb-1">
                              {arch.name} ({arch.probability})
                            </label>
                            <input
                              type="range"
                              min="0"
                              max="100"
                              value={arch.probability}
                              onChange={(e) => {
                                const newArchetypes = [...archetypes];
                                newArchetypes[index].probability = parseInt(
                                  e.target.value,
                                );
                    ...
                    <div className="mb-6">
                      ...
                      <div className="space-y-2">
                        {imageResolutions.map((res, i) => (
                          <div key={i} className="flex gap-2 items-center">
                            ...
                            <button
                              onClick={() => {
                                const newRes = imageResolutions.filter(
                                  (_, idx) => idx !== i,
                                );
                                setImageResolutions(newRes);
                              }}
                        ...
                        <div className="flex gap-2 mt-2">
                          <button
                            ...
                            className="bg-slate-800 hover:bg-slate-700 text-slate-100 py-2 px-4 rounded-lg transition flex items-center justify-center gap-2"
                          >
                            <Plus size={16} /> Add Resolution
                          </button>
                          <button
                            onClick={() =>
                              saveImageResolutions(imageResolutions)
                            }
                            className="bg-orange-600 hover:bg-orange-500 text-white font-bold py-2 px-4 rounded-lg transition ml-auto"
                          >
                            Save
                          </button>
                        </div>
                      ...
                      <p className="text-xs text-slate-500 mt-2">
                        A random resolution from this list will be picked for
                        image generation.
                      </p>
                    </div>
                    ...
                    <div className="mb-6">
                      <label className="block text-sm font-medium text-slate-400 mb-1">
                        Vision Model (for user image uploads)
                      </label>
                      <div className="flex gap-2">
                        <input
                          type="text"
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <button
                        ...
                        className="text-sm text-slate-500 hover:text-orange-500 transition"
                      >
                        Refresh Logs
                      </button>
                    </div>
                    ...
                    {testResult && (
                      <div
                        className={`mt-4 p-4 rounded-lg ${testResult.success ? "bg-green-500/10 text-green-400 border border-green-500/30" : "bg-red-500/10 text-red-400 border border-red-500/30"}`}
                      >
                        {testResult.success ? (
                          <p className="text-sm">
                ...
                {loggedInUser?.role === "admin" && (
                  <section className="bg-slate-900/50 border border-red-500/30 p-6 rounded-2xl backdrop-blur-sm">
                    ...
                    <p className="text-sm text-slate-400 mb-4">
                      Resetting the database will delete data. This action
                      cannot be undone.
                    </p>
                    <div className="flex gap-4">
                      <button
                ...
                {loggedInUser?.role === "admin" && (
                  <section className="bg-slate-900/50 border border-white/10 p-6 rounded-2xl backdrop-blur-sm">
                    ...
                    <p className="text-sm text-slate-400 mb-4">
                      Force a random AI character to generate a new post
                      immediately.
                    </p>
                    <div className="flex gap-4">
                      <button
                        onClick={async () => {
                          ...
                          if (aiUsers.length === 0)
                            return showToast("No AI characters available");
                          const randomUser =
                            aiUsers[Math.floor(Math.random() * aiUsers.length)];
                ...
                {loggedInUser?.role === "admin" && (
                  <section className="bg-slate-900/50 border border-white/10 p-6 rounded-2xl backdrop-blur-sm">
                    ...
                    <p className="text-sm text-slate-500 mb-4">
                      View recent API calls to NanoGPT for troubleshooting.
                    </p>
                    <button
                      onClick={() => {
                        fetchApiLogs();
          ...
          {activeTab === "profile" && editingProfile && (
            <div className="p-6 max-w-2xl mx-auto">
              <h2 className="text-2xl font-bold mb-6 text-slate-100">
                Edit Profile: {editingProfile.display_name}
              </h2>
                  ...
                  <>
                    <div>
                      ...
                      <input
                        ...
                        type="password"
                        maxLength={4}
                        className="w-full bg-slate-900/50 border border-white/10 rounded-lg p-3 text-slate-100 outline-none focus:border-orange-500 backdrop-blur-sm placeholder-slate-500"
                        placeholder="4-digit PIN"
                      />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-slate-400 mb-1">
                        DM Frequency from AI Characters
                      </label>
                      <select
                        value={profileDmFrequency}
                        onChange={(e) => setProfileDmFrequency(e.target.value)}
                        className="w-full bg-slate-900/50 border border-white/10 rounded-lg p-3 text-slate-100 outline-none focus:border-orange-500 backdrop-blur-sm"
                      >
                      ...
                      <p className="text-xs text-slate-500 mt-1">
                        Controls how often AI characters you follow will
                        initiate DMs with you.
                      </p>
                    </div>
                  </>
                )}
                <div>
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      type="text"
                      className="flex-1 bg-slate-900/50 border border-white/10 rounded-lg p-3 text-slate-100 outline-none focus:border-orange-500 backdrop-blur-sm placeholder-slate-500"
                      placeholder="https://..."
                    />
                  ...
                  <div className="flex flex-col gap-2">
                    {profileReferenceImages.map((img, idx) => (
                      <div key={idx} className="flex gap-2 items-center">
                        ...
                        <input
                          ...
                          type="text"
                          className="flex-1 bg-slate-900/50 border border-white/10 rounded-lg p-2 text-slate-100 outline-none focus:border-orange-500 backdrop-blur-sm placeholder-slate-500"
                          placeholder="Image URL..."
                        />
                        <button
                          type="button"
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="bg-slate-800 hover:bg-slate-700 text-slate-100 px-4 py-2 rounded-lg flex items-center gap-2 text-sm border border-white/10 transition-colors"
                      >
                        <Plus size={16} /> Add URL
                      </button>
                  ...
                  <div className="mt-8 pt-6 border-t border-white/10">
                    ...
                    <div className="space-y-4">
                      ...
                      {profileAccountType === "company" && (
                        <div className="space-y-4 bg-slate-950/50 p-4 rounded-xl border border-white/10 mb-6">
                          ...
                          <div>
                            ...
                            <textarea
                              value={profileBrandIdentity}
                              onChange={(e) =>
                                setProfileBrandIdentity(e.target.value)
                              }
                              rows={2}
                              className="w-full bg-slate-900/50 border border-white/10 rounded-lg p-3 text-slate-100 outline-none focus:border-orange-500 backdrop-blur-sm"
                            ></textarea>
                          ...
                          <div>
                            ...
                            <textarea
                              ...
                              onChange={(e) =>
                                setProfileProductsServices(e.target.value)
                              }
                              rows={2}
                              className="w-full bg-slate-900/50 border border-white/10 rounded-lg p-3 text-slate-100 outline-none focus:border-orange-500 backdrop-blur-sm"
                            ></textarea>
                          ...
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                setProfileTargetAudience(e.target.value)
                              }
                              type="text"
                              className="w-full bg-slate-900/50 border border-white/10 rounded-lg p-3 text-slate-100 outline-none focus:border-orange-500 backdrop-blur-sm"
                            />
                          ...
                          <div>
                            ...
                            >
                              ...
                              {users
                                .filter(
                                  (u) => u.is_ai && u.id !== editingProfile.id,
                                )
                                .map((u) => (
                                  <option key={u.id} value={u.id}>
                                    {u.display_name} (@{u.username})
                      ...
                      {profileAccountType === "news" && (
                        <div className="space-y-4 bg-slate-950/50 p-4 rounded-xl border border-white/10 mb-6">
                          <h4 className="font-bold text-orange-400 mb-2 flex items-center gap-2">
                            <Globe size={16} /> News Account Details
                          </h4>
                          <p className="text-sm text-slate-400">
                            News accounts post daily summaries of events in
                            their universe. They do not form relationships or
                            comment on posts.
                          </p>
                        </div>
                      )}
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          onChange={(e) =>
                            setProfileWritingStyle(e.target.value)
                          }
                          rows={3}
                          className="w-full bg-slate-900/50 border border-white/10 rounded-lg p-3 text-slate-100 outline-none focus:border-orange-500 backdrop-blur-sm"
                        ></textarea>
                      ...
                      {profileAccountType === "character" && (
                        <>
                          <div>
                            ...
                            <textarea
                              value={profilePhysicalAppearance}
                              onChange={(e) =>
                                setProfilePhysicalAppearance(e.target.value)
                              }
                              rows={2}
                              className="w-full bg-slate-900/50 border border-white/10 rounded-lg p-3 text-slate-100 outline-none focus:border-orange-500 backdrop-blur-sm"
                            ></textarea>
                          ...
                          <div>
                            ...
                            <textarea
                              value={profileClothingStyle}
                              onChange={(e) =>
                                setProfileClothingStyle(e.target.value)
                              }
                              rows={2}
                              className="w-full bg-slate-900/50 border border-white/10 rounded-lg p-3 text-slate-100 outline-none focus:border-orange-500 backdrop-blur-sm"
                            ></textarea>
                        ...
                        <div>
                          <label className="flex items-center gap-2 cursor-pointer">
                            <input
                              ...
                              onChange={(e) =>
                                setProfileIsVerified(e.target.checked)
                              }
                              className="w-4 h-4 bg-slate-900 border border-white/10 rounded accent-orange-500"
                            />
                            <span className="text-sm font-medium text-slate-400 group-hover:text-slate-200 transition-colors">
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 bg-slate-950/50 p-4 rounded-xl border border-white/10">
                          {ONLINE_TIME_WINDOWS.map((window) => (
                            ...
                            >
                              <input
                                type="checkbox"
                                checked={profileOnlineTimes.includes(
                                  window.value,
                                )}
                                onChange={(e) => {
                                  if (e.target.checked) {
                                    setProfileOnlineTimes((prev) => [
                                      ...prev,
                                      window.value,
                                    ]);
                                  } else {
                        ...
                        <p className="text-xs text-slate-500 mt-2 italic">
                          If no window is selected, the character is online
                          24/7.
                        </p>
                      </div>
                      {profileAccountType !== "news" && (
                ...
                <div className="pt-6 border-t border-white/10">
                    ...
                    {profileRelationships.map((rel) => (
                      ...
                      >
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            handleDeleteRelationship(rel.user_id_2)
                          }
                  ...
                  <div className="flex gap-2 items-end">
                        ...
                        {users
                          .filter((u) => {
                            ...
                            const isEditingCompany =
                              editingProfile.account_type === "company";
                            const isTargetCompany =
                              u.account_type === "company";

                            if (isEditingCompany && !isTargetCompany)
                              return false;
                            if (
                              (isEditingCompany || isTargetCompany) &&
                              editingProfile.universe_id !== u.universe_id
                            )
                              return false;
                    ...
                    <div className="flex-[2]">
                      <label className="block text-xs font-medium text-slate-400 mb-1">
                        Description (e.g. "is best friends with")
                      </label>
                      <input
                        type="text"
                        value={newRelDesc}
                ...
                <div className="flex gap-4 pt-4">
                  {editingProfile.is_ai === 1 && (
                    <button
                      ...
                      className="flex-1 bg-red-600/20 text-red-500 border border-red-500/30 font-bold py-3 rounded-full hover:bg-red-600/30 transition-colors"
                    >
                      Delete Character
                    </button>
                  )}
                  <button
                    ...
                    className="flex-1 bg-slate-800 text-slate-100 font-bold py-3 rounded-full hover:bg-slate-700 transition-colors border border-white/10"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="flex-1 bg-orange-600 text-white font-bold py-3 rounded-full hover:bg-orange-500 transition-colors shadow-lg shadow-orange-500/20"
                  >
                    Save Profile
                  </button>
                </div>
        ...
        <div className="md:hidden fixed bottom-0 left-0 right-0 bg-slate-950/90 backdrop-blur-xl border-t border-white/10 z-40 flex justify-around items-center p-2 pb-safe">
          <button
            ...
            className={`p-2 rounded-xl flex flex-col items-center gap-1 ${activeTab === "home" ? "text-orange-500" : "text-slate-400 hover:text-slate-200"}`}
          >
            <Home size={20} />
          ...
          <button
            ...
            className={`p-2 rounded-xl flex flex-col items-center gap-1 ${activeTab === "fauxpics" ? "text-orange-500" : "text-slate-400 hover:text-slate-200"}`}
          >
            <Camera size={20} />
          ...
          <button
            ...
            className={`p-2 rounded-xl flex flex-col items-center gap-1 ${activeTab === "news" ? "text-orange-500" : "text-slate-400 hover:text-slate-200"}`}
          >
            <Newspaper size={20} />
          ...
          <button
            ...
            className={`p-2 rounded-xl flex flex-col items-center gap-1 relative ${activeTab === "notifications" ? "text-orange-500" : "text-slate-400 hover:text-slate-200"}`}
          >
            <div className="relative">
              <Bell size={20} />
              {unreadNotifs > 0 && (
          ...
          <button
            onClick={() => setShowMobileMenu(true)}
            className={`p-2 rounded-xl flex flex-col items-center gap-1 ${["messages", "universes", "following", "explore", "arcs", "relationships", "fauxpast", "settings"].includes(activeTab) ? "text-orange-500" : "text-slate-400 hover:text-slate-200"}`}
          >
            <Menu size={20} />
          ...
          <div className="md:hidden fixed inset-0 z-50 flex">
            ...
            <div className="relative w-64 bg-slate-950 border-r border-white/10 h-full flex flex-col animate-in slide-in-from-left duration-200">
              ...
              <div className="p-4 border-t border-white/10">
                <div
                  onClick={() => {
                    handleEditProfile(loggedInUser);
                    setShowMobileMenu(false);
                  }}
        ...
        {viewingPostData && (
          <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
            <div className="bg-slate-900 border border-white/10 rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-y-auto relative shadow-2xl">
              <button
                onClick={() => setViewingPostData(null)}
                className="absolute top-4 right-4 text-slate-400 hover:text-white transition-colors z-10 bg-slate-800/50 p-2 rounded-full backdrop-blur-sm border border-white/10"
              >
                <X size={24} />
              </button>
              ...
              <div className="px-6 pb-6">
                <div className="relative -mt-12 mb-4">
                  ...
                  >
                    {viewingProfile.avatar_url ? (
                      ...
                      <User size={48} className="m-auto mt-4 text-slate-500" />
                    )}
                  </div>
                  {viewingProfile.is_ai === 1 && (
                    <div
                      className={`absolute bottom-1 left-1 w-6 h-6 rounded-full border-4 border-slate-900 ${isUserOnline(viewingProfile) ? "bg-green-500 shadow-[0_0_12px_rgba(34,197,94,0.8)]" : "bg-slate-500"}`}
                      title={
                ...
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {viewingProfile.is_ai === 1 && (
                        <span
                          className={`text-[10px] uppercase tracking-widest font-black px-2 py-0.5 rounded-full ${isUserOnline(viewingProfile) ? "bg-green-500/20 text-green-400 border border-green-500/30" : "bg-slate-500/20 text-slate-400 border border-slate-500/30"}`}
                        >
                          {isUserOnline(viewingProfile) ? "Online" : "Offline"}
                        </span>
                      )}
                    ...
                    {viewingProfile.universe_id &&
                      universes.find(
                        (u) => u.id === viewingProfile.universe_id,
                      ) && (
                        <div
                          onClick={() =>
                            handleViewUniverse(viewingProfile.universe_id)
                          }
                      ...
                      <button
                        onClick={() => {
                          ...
                          const existingChat = conversations.find(
                            (c) => c.other_user.id === viewingProfile.id,
                          );
                          if (existingChat) {
                            setActiveChat(existingChat);
                            setIsGroupChat(false);
                            const isAlreadyOpen =
                              activeChat?.id === existingChat.id &&
                              !isGroupChat;
                            fetchChatMessages(
                              existingChat.id,
                              false,
                ...
                {viewingProfile.account_type === "company" && (
                  <div className="mb-4 space-y-2 text-sm bg-slate-950/50 p-4 rounded-xl border border-white/10">
                    ...
                    {viewingProfile.run_by_character_id &&
                      users.find(
                        (u) => u.id === viewingProfile.run_by_character_id,
                      ) && (
                          ...
                          >
                            {
                              users.find(
                                (u) =>
                                  u.id === viewingProfile.run_by_character_id,
                              )?.display_name
                            }
                          </span>
                        </p>
                ...
                {(viewingProfile.account_type === "news" ||
                  ...
                  <div className="mb-4 space-y-2 text-sm bg-slate-950/50 p-4 rounded-xl border border-white/10">
                    <div className="flex items-center gap-2 text-orange-400 font-bold mb-2">
                      <Globe size={16} />{" "}
                      {viewingProfile.account_type === "faux_news"
                        ? "Platform News Account"
                        : "Universe News Account"}
                    </div>
                ...
                <div className="flex gap-4 text-sm text-slate-500 mb-6">
                  <span
                    ...
                    onClick={async () => {
                      const res = await apiFetch(
                        `/api/users/${viewingProfile.id}/following`,
                      );
                      const data = await res.json();
                  ...
                  >
                    <strong className="text-slate-100">
                      {viewingProfile.following_count || 0}
                    </strong>{" "}
                    Following
                  </span>
                  <span
                    ...
                    onClick={async () => {
                      const res = await apiFetch(
                        `/api/users/${viewingProfile.id}/followers`,
                      );
                      const data = await res.json();
                  ...
                  >
                    <strong className="text-slate-100">
                      {viewingProfile.follower_count || 0}
                    </strong>{" "}
                    Followers
                  </span>
                </div>
                        ...
                        <>
                          {viewingProfilePosts
                            ...
                            .map((post) => (
                              <PostItem
                                ...
                                post={{
                                  ...post,
                                  display_name: viewingProfile.display_name,
                                ...
                                formatTimestamp={formatTimestamp}
                                onRefresh={() =>
                                  handleViewProfile(viewingProfile.id)
                                }
                                onDelete={(id) => {
                                  setViewingProfilePosts((prev) =>
                          ...
                          {viewingProfilePosts.length > visibleProfilePosts && (
                            <div className="flex justify-center py-4">
                              <button
                                onClick={() =>
                                  setVisibleProfilePosts((prev) => prev + 30)
                                }
                        ...
                        <>
                          <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                            {viewingProfilePosts
                              .filter(
                                (p) =>
                                  p.post_type === "image_post" && p.image_url,
                              )
                              .map((post) => (
                                <div
                                  key={post.id}
                          ...
                          {viewingProfilePosts.filter(
                            (p) => p.post_type === "image_post" && p.image_url,
                          ).length === 0 && (
                            <div className="text-center py-12 text-slate-500">
                              <Camera
                                size={48}
                    ...
                    <div className="space-y-4">
                        ...
                        (() => {
                          ...
                          return (
                            <div className="flex gap-2 mb-4">
                              <button
                                onClick={() =>
                                  handleAddArc("character", viewingProfile.id)
                                }
                                disabled={isPaused}
                                title={
                                  isPaused
                                    ? "Cannot add arcs to a character in a paused universe"
                                    : ""
                                }
                              ...
                              <button
                                ...
                                title={
                                  isPaused
                                    ? "Cannot generate arcs for a character in a paused universe"
                                    : ""
                                }
                      ...
                      {viewingProfileArcs.length === 0 ? (
                        ...
                        viewingProfileArcs.map((arc) => (
                          <div
                            key={arc.id}
                            className="bg-slate-900/50 border border-white/10 rounded-xl p-4 backdrop-blur-sm"
                          >
                            <div className="flex justify-between items-start mb-2">
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <span
                                  className={`text-xs font-bold px-2 py-1 rounded-full ${arc.status === "active" ? "bg-green-500/20 text-green-400 border border-green-500/30" : "bg-slate-800 text-slate-400 border border-white/10"}`}
                                >
                                  {arc.status.toUpperCase()}
                                </span>
                              </div>
                            </div>
                            <p className="text-slate-400 text-sm mb-4 whitespace-pre-wrap">
                              {arc.description}
                            </p>
                            {arc.status === "completed" &&
                              arc.completion_summary && (
                  ...
                  >
                    ...
                    >
                      {u.avatar_url ? (
                        ...
                        <User
                          size={20}
                          className="m-auto mt-2 text-slate-500"
                        />
                      )}
                    </div>
                    <div>
            ...
            <div className="bg-slate-900 border border-white/10 rounded-2xl p-6 w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
              ...
              <div className="mb-4 flex gap-2">
                <input
                  ...
                  onKeyDown={(e) =>
                    e.key === "Enter" && fetchApiLogs(apiLogSearch)
                  }
                ...
                {apiLogs.length === 0 ? (
                  ...
                  apiLogs.map((log: any) => {
                    ...
                    if (
                      ...
                      (log.response_payload.includes("Error:") ||
                        log.response_payload.includes('"error"'))
                    ) {
                      isError = true;
                    }
                    ...
                    return (
                      <div
                        key={log.id}
                        className={`bg-slate-900/50 rounded-xl text-xs font-mono border ${isError ? "border-red-500/30" : "border-green-500/30"} overflow-hidden transition-colors backdrop-blur-sm`}
                      >
                        <div
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <div className="font-bold text-slate-100 text-sm flex items-center gap-2">
                                {log.endpoint}
                                {isError && (
                                  <span className="bg-red-500/20 text-red-400 border border-red-500/30 text-[9px] px-1.5 py-0.5 rounded uppercase tracking-wider">
                                    Error
                                  </span>
                                )}
                              </div>
                        ...
                        {isExpanded && (
                          <div className="p-4 border-t border-white/10 bg-slate-950/50">
                            <div className="mb-3 space-y-1">
                              <div className="text-slate-500 uppercase text-[9px] tracking-wider font-bold">
                                Request Details
                              </div>
                              <div className="bg-black/30 p-2 rounded border border-white/5 overflow-x-auto whitespace-pre-wrap">
                                {Object.entries(requestObj).map(
                                  ([key, val]) => (
                                    ...
                                    <div className="space-y-2">
                                      {resObj.reasoning && (
                                        <div className="bg-blue-900/10 border border-blue-500/20 p-2 rounded">
                                          <div className="text-[10px] text-blue-400 font-bold mb-1 uppercase tracking-tighter">
                                            Thinking / Reasoning
                                          </div>
                                      ...
                                      <div
                                        className={`p-2 rounded border ${!resObj.content ? "bg-red-900/20 border-red-500/30 text-red-200" : "bg-green-900/20 border-green-500/30 text-green-200"} whitespace-pre-wrap overflow-x-auto`}
                                      >
                                        <div className="text-[10px] opacity-50 font-bold mb-1 uppercase tracking-tighter">
                                          Final Output
                                        </div>
                      ...
                      .map((user) => (
                        ...
                        >
                          ...
                          >
                            ...
                            {user.is_ai === 1 ? (
                              <div
                                className={`absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full border border-slate-900 ${isUserOnline(user) ? "bg-green-500 shadow-[0_0_4px_rgba(34,197,94,0.6)]" : "bg-slate-500"}`}
                                title={
                                  isUserOnline(user) ? "Online" : "Offline"
                                }
                              ></div>
                              ...
                              <div
                                className="absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full border border-slate-900 bg-blue-500"
                                title="Real User"
                              ></div>
                            )}
                          </div>
...
function renderContentWithTags(
  content: string,
  users: any[] | undefined,
...
const FauxPicItem = React.memo(function FauxPicItem({
  post,
  onLike,
  onViewProfile,
  onShowLikers,
  ...
  onLike: () => void;
  onViewProfile: (id: number) => void;
  onShowLikers: (type: "post" | "comment", id: number) => void;
    ...
    try {
      await apiFetch(`/api/posts/${post.id}/comments`, {
        ...
        body: JSON.stringify({
          ...
          parent_id: newReplyObj.parent_id,
        }),
      });
      fetchComments();
    } catch (e) {
  ...
  return (
    <div className="bg-slate-900/80 border border-white/10 rounded-2xl overflow-hidden shadow-2xl backdrop-blur-sm">
      ...
      <div className="p-4">
        <div className="flex gap-4 mb-3 items-center">
          <div className="flex items-center gap-2">
            ...
            {post.like_count > 0 && (
              <button
                onClick={() => onShowLikers("post", post.id)}
                className="text-sm font-bold hover:underline text-slate-300"
              >
                {post.like_count}
              </button>
            )}
          ...
          <Send
            size={24}
            className="text-slate-300 hover:text-white cursor-pointer hover:scale-110 transition-transform"
          />
        </div>
        <p className="text-sm text-slate-300">
          <span
            className="font-bold mr-2 cursor-pointer hover:underline text-slate-100"
            onClick={() => onViewProfile(post.user_id)}
          >
            {post.display_name}
          </span>
          ...
          <div className="mt-4 pt-4 border-t border-white/10 space-y-4">
              ...
              {rootComments.length === 0 ? (
                ...
                rootComments.map((comment) => (
                  <CommentItem
                    ...
                    onReply={(c) =>
                      setReplyingTo({ id: c.id, name: c.display_name })
                    }
            ...
            <form onSubmit={handleAddComment} className="flex gap-2 mt-4">
              ...
              <button
                ...
                className="text-orange-500 font-bold text-sm disabled:opacity-50 hover:text-orange-400 transition-colors"
              >
                Post
              </button>
            </form>
      ...
      {replyingTo && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-white/10 rounded-2xl w-full max-w-md p-6 shadow-2xl">
            <h3 className="font-bold mb-4 text-slate-100">
            ...
            <form onSubmit={handleAddReply}>
              ...
              <div className="flex justify-end gap-3">
                ...
                <button
                  ...
                  className="px-6 py-2 bg-orange-600 hover:bg-orange-500 text-white rounded-lg font-bold transition-colors disabled:opacity-50 shadow-lg shadow-orange-500/20 disabled:shadow-none"
                >
                  Reply
                </button>
              </div>
...
const PostItem = React.memo(function PostItem({
  ...
  loggedInUser?: any;
  apiFetch: any;
  onViewApiLogs?: (content: string) => void;
}) {
  const [showComments, setShowComments] = useState(false);
            ...
            {showMenu && (
              <div className="absolute right-0 top-6 bg-slate-900 border border-white/10 rounded-lg shadow-2xl z-10 w-36 overflow-hidden backdrop-blur-md">
                <button
                  ...
                  className="w-full text-left px-4 py-2 text-sm hover:bg-slate-800 transition-colors text-slate-300 hover:text-slate-100"
                >
                  Edit
                </button>
                <button
                ...
                {loggedInUser?.role === "admin" && onViewApiLogs && (
                  <button
                    onClick={() => {
                      onViewApiLogs(post.content);
                      setShowMenu(false);
                    }}
                    className="w-full text-left px-4 py-2 text-sm text-blue-400 hover:bg-blue-500/10 transition-colors border-t border-white/10"
                  >
                    View API Logs
                  </button>
                )}
          ...
          {isEditing ? (
            <div className="mt-2">
              ...
              <div className="flex justify-end gap-2 mt-2">
                ...
                <button
                  onClick={handleEdit}
                  className="px-3 py-1 text-sm bg-orange-600 hover:bg-orange-500 text-white rounded-full transition-colors shadow-lg shadow-orange-500/20"
                >
                  Save
                </button>
              </div>
          ...
          <div className="flex gap-12 mt-3 text-slate-500">
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                className={`flex items-center gap-2 hover:text-pink-500 transition-colors ${post.is_liked ? "text-pink-500" : ""}`}
              >
                <Heart
                  size={18}
              ...
              {post.like_count > 0 && (
                <button
                  onClick={() => onShowLikers("post", post.id)}
                  className="text-[10px] hover:underline hover:text-slate-300 transition-colors"
                >
                  view
                </button>
              )}
        ...
        <div className="mt-4 pl-10 space-y-4 border-l-2 border-white/10 ml-5">
          ...
          {rootComments.map((comment) => (
            <CommentItem
              ...
              onLike={handleCommentLike}
              onReply={(c) =>
                setReplyingTo({ id: c.id, name: c.display_name, content: "" })
              }
              onViewProfile={onViewProfile}
              onShowLikers={onShowLikers}
              formatTimestamp={formatTimestamp}
          ...
          >
            ...
            <button
              type="submit"
              disabled={!newComment.trim()}
              className="text-orange-500 font-bold text-sm disabled:opacity-50 mb-1 hover:text-orange-400 transition-colors"
            >
              Reply
            </button>
          </form>
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-white/10 rounded-2xl w-full max-w-md p-6 shadow-2xl">
            ...
            <TagTextarea
              ...
              onValueChange={(val) =>
                setReplyingTo({ ...replyingTo, content: val })
              }
            ...
            <div className="flex gap-3">
              <button
                onClick={() => setReplyingTo(null)}
                className="flex-1 bg-slate-800 hover:bg-slate-700 text-slate-100 font-bold py-2 rounded-full transition-colors border border-white/10"
              >
                Cancel
              </button>
              <button
                onClick={(e) => handleAddComment(e as any, replyingTo.id)}
                className="flex-1 bg-orange-600 hover:bg-orange-500 text-white font-bold py-2 rounded-full transition-colors shadow-lg shadow-orange-500/20"
              >
                Reply
              </button>
            </div>
...
const CommentItem = React.memo(function CommentItem({
  ...
  onDelete?: (id: number) => void;
  highlightedCommentId?: number | null;
  commentRef?: (id: number, el: HTMLDivElement | null) => void;
  users?: any[];
  loggedInUser?: any;
  apiFetch: any;
  onViewApiLogs?: (content: string) => void;
  ...
  return (
    <div
      className="space-y-3"
      ref={(el) => commentRef && commentRef(comment.id, el)}
    >
      <div
        className={`flex gap-3 group p-2 -m-2 rounded-xl transition-colors ${highlightedCommentId === comment.id ? "bg-orange-900/20 ring-2 ring-orange-500/50 shadow-[0_0_15px_rgba(249,115,22,0.3)]" : "hover:bg-slate-900/30"}`}
      >
        <div
            ...
            {isEditing ? (
              <div className="mt-2">
                ...
                <div className="flex justify-end gap-2 mt-2">
                  ...
                  <button
                    onClick={handleEdit}
                    className="px-3 py-1 text-xs bg-orange-600 hover:bg-orange-500 text-white rounded-full transition-colors shadow-lg shadow-orange-500/20"
                  >
                    Save
                  </button>
                </div>
              ...
              <p className="text-sm whitespace-pre-wrap text-slate-300">
                {renderContentWithTags(comment.content, users, onViewProfile)}
              </p>
            )}
          ...
          <div className="flex gap-6 mt-1 ml-2 text-slate-500">
            <div className="flex items-center gap-1">
              ...
              {comment.like_count > 0 && (
                <button
                  onClick={() => onShowLikers("comment", comment.id)}
                  className="text-[9px] hover:underline hover:text-slate-300 transition-colors"
                >
                  view
                </button>
              )}
            ...
            <button
              ...
              className="text-xs hover:text-orange-500 transition-colors font-bold"
            >
              Reply
            </button>
          </div>
